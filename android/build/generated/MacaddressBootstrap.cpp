...
#define TAG "com.ibilis.macaddress"

using namespace v8;

static Persistent<Object> bindingCache;

static void Macaddress_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "Macaddress.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = titanium::bindings::MacaddressBindings::lookupGeneratedInit(
...
static void Macaddress_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, Macaddress_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void Macaddress_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			titanium::bindings::MacaddressBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry MacaddressBinding = {
	"com.ibilis.macaddress",
	Macaddress_init,
	Macaddress_dispose
};
...
Java_com_ibilis_macaddress_MacaddressBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("com.ibilis.macaddress", &MacaddressBinding);
	titanium::KrollBindings::addExternalLookup(&(titanium::bindings::MacaddressBindings::lookupGeneratedInit));
}
